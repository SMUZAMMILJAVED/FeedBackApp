...
const DashBoardComp = () => {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        const res = await axios.get("/api/feedback/getfeedbacks?limit=5");
        if (res.status === 200 || res.status === 201) {
          const data = res.data;
          console.log("data----------checkdata----->", data);
          setPosts(data.feedBacks);
          setTotalPosts(data.totalFeedBacks);
          setLastMonthPosts(data.lastMonthFeedBacks);
        }
      } catch (error) {
        console.log(error.message);
      }
            ...
            {posts &&
              posts.map((post) => (
                <Table.Body key={post._id} className="divide-y">
                  <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                    <Table.Cell>
                      <p className="line-clamp-2">{post.feedback}</p>
                    </Table.Cell>
                    <Table.Cell className="w-96">{post.user}</Table.Cell>
